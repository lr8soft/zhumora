...
A bundled CPU text motion model also accepts short Chinese or English descriptions through `avatar_control` with `action: "generate_motion"`. Its current vocabulary covers idle, speaking, waving, greeting, pointing, celebration, surprise, using a phone, drinking, encouragement, bowing and spinning, and unsupported descriptions return an error. Only the intents inside that vocabulary (idle, greeting, waving, celebration, bowing) are driven by the model; every other intent keeps its procedural motion, and the system prompt advertises only the covered intents. Offline CUDA training and model details are in [`scripts/avatar_motion/README.md`](./scripts/avatar_motion/README.md).
