...
Avatars include application-owned idle, thinking, explaining, nodding, head-shaking, waving and celebration motions, adapted to VRM 0/1. They blink and vary their idle pose without LLM calls. Agent activity drives thinking and response gestures; one-shot gestures return to the current activity with blended transitions. Mouse gaze follows only inside the Avatar window and smoothly returns forward on exit. Expressions fade in and return to neutral automatically.
