...
export default {
  ...
  settings: {
    ...
    mcpServer: {
      ...
      port: 'ポート',
      portHint: '0 = 自動割当。固定ポートはクライアントに安定したアドレスを与えます。使用中のポートは自動的に一時ポートへ切り替わります。',
      token: 'アクセストークン',
      tokenPlaceholder: '空欄 = 起動ごとに自動生成',
      tokenHint: 'クライアントはこのトークンで認証します。「生成」で固定トークンを書き込むと、設定保存後に再起動しても変わりません。',
      tokenWaitHint: 'トークンはサーバーが保存・起動されてから生成されます。「生成」を押すとすぐ固定トークンが得られます。',
      generateToken: '生成',
      copyToken: 'コピー',
      autoTokenNote: '現在のトークンは自動生成で保存されていません。アプリ再起動で変わるので再コピーしてください。「生成」で固定トークンを書き込めます。',
      label: 'クライアント表示名',
      ...
      copyJson: '設定をコピー',
      jsonHint: 'トークンやポートを変更した場合は、この設定を再生成してください。',
