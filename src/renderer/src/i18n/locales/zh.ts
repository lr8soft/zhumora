...
export default {
  ...
  settings: {
    ...
    mcpServer: {
      ...
      token: '访问令牌',
      tokenPlaceholder: '留空 = 每次启动自动生成',
      tokenHint: '客户端用它鉴权。点「生成」可写入一个固定令牌（保存设置后跨重启不变）。',
      tokenWaitHint: '令牌在服务器保存并启动后才生成；点「生成」可立即得到一个固定令牌。',
      generateToken: '生成',
      copyToken: '复制',
      autoTokenNote: '当前令牌为自动生成，未写入设置；重启应用后令牌会变化，需重新复制。点「生成」可写入固定令牌。',
      label: '客户端名称',
