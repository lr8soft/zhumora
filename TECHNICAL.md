...
Avatar presentation state belongs to each window. Its event adapter subscribes to `SessionEventHub` and projects session-tagged running/assistant/tool events into idle/thinking/speaking. The motion controller owns bounded semantic overrides, default idle, autonomous variation and interruptible weight blending. Built-in gestures are procedural pose curves over the normalized humanoid bones: head/torso cues plus wave, shrug, bow and applause arm motions. Original built-in quaternion clips pass through VRM0 coordinate conversion; imported VRMA uses the library retargeter. A procedural life-motion layer (`AvatarLifeMotion`) sandwiches additive breathing, weight-shift, arm-sway and finger-flexion overlays around mixer updates, so ambient motion composes with any clip (including imported VRMA idles) without accumulating on untracked bones. Expression fading/blinking has a separate owner. Semantic animation mappings persist at the normalized settings boundary; capability reports include validated clips and semantic intents before prompt construction. Animation/expressions never create conversation history or invoke other tools.

The optional local text motion model adds a bounded `generate_motion` action to the same `avatar_control` normal-permission tool. A sandboxed Avatar renderer worker runs a small exported PyTorch pose decoder with plain CPU `Float32Array` math. It reports its trained vocabulary in a startup handshake, and only the intents that appear in that vocabulary may replace a built-in motion: an untrained intent would otherwise be answered by an unrelated clip, so every other intent keeps its procedural motion. Short Chinese/English action phrases route to twelve trained actions; the decoder samples normalized VRM humanoid quaternions, then the renderer creates a standard Three.js clip and applies the existing VRM0 conversion and motion blending. User-configured VRMA overrides take precedence and unsupported text returns an error. The reported vocabulary becomes the `textMotionActions` capability, and `trainedAvatarIntents` derives the intents the system prompt may advertise. The worker is per Avatar window, does not own session/run state or call IPC, and is terminated with the window. Training uses local CUDA offline; source VRMA files and Python are never bundled.
...
- `protocol.ts` — builds the MCP server and its tools. Discovery is layered: initialize `instructions`, a `delegate-to-zhumora` prompt for hosts that support prompts, and per-tool descriptions that lead with *when* to delegate, inline with concrete task examples and a negative scope. Most hosts inject MCP `instructions` unreliably (opencode has no per-server channel; Codex only newer versions, and never consumes prompts) — the `zhumora_chat` description is the only text Codex and opencode models ever see, so the delegation-willingness wording (same-machine capability, typical task examples, negative scope) lives in both `SERVER_INSTRUCTIONS` (composed from `DELEGATION_IDENTITY` / `DELEGATION_PROTOCOL` in `src/shared/mcpServer.ts`) and the tool description; tests lock the same element set in both places.
